/*
	...
	Add config defined inventory to an unit
		(Aloe: used global commands for add Inventory)
	...
	if (_isCfg) then {
		...
		_magazines = getarray (_cfg >> "magazines");
		{
			if (_x != "") then {
				...
				_object addmagazine [_magazine, 1];
			};
		} foreach _magazines;
	} else {
		...
		if ({!isnil "_x"} count (_inventory select 6) > 2) then {
			{
				if (_x != "") then {_object addmagazine [_x, 1];};
...
if !(_isCfg) then {
	/*
	diag_log format ["loadInventoryGlobal.sqf: _inventorty: %1", str _inventory];
	diag_log format ["loadInventoryGlobal.sqf: _isCfg: %1", str _isCfg];
	diag_log format ["loadInventoryGlobal.sqf: _object: %1", str _object];
	diag_log format ["loadInventoryGlobal.sqf: typeName: %1", typeName _object];
	diag_log format ["loadInventoryGlobal.sqf: local: %1", local _object];
	*/
	//--- Add container items (only after weapons were added together with their default magazines)
	if !("uniform" in _blacklist) then {
		{
			(uniformContainer _object) addItemCargoGlobal [_x,1];
		} foreach ((_inventory select 0) select 1);
	};
	if !("vest" in _blacklist) then {
		{
			(vestContainer _object) addItemCargoGlobal [_x,1];
		} foreach ((_inventory select 1) select 1);
	};
	if !("backpack" in _blacklist) then {
		{
			(backpackContainer _object) addItemCargoGlobal [_x,1];
