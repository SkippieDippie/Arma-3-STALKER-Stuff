...
*/
	...
	if (_CHANCE >= 2) then 	{
		...
		{	
			player removemagazine "Item_ELECTRA_Unstab";
			_fatigue = getFatigue player;
			player setFatigue (_fatigue + 1);
			player setVariable ["AGM_Blood", (player getVariable "AGM_Blood") - 0.9];
			player setVariable ["ARMST_THIRST", (player getVariable "ARMST_THIRST") - 0.5];
