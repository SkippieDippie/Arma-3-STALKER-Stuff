	...
	if (_CHANCE >= 4) then 	{
		_a=0;
  		while {_a < 60} do 	{
			...
			player setVariable ["AGM_Blood", (player getVariable "AGM_Blood") + 0.001];
			player setVariable ["AGM_Pain", (player getVariable "AGM_Pain") - 0.01];
		...
		{	
			player removemagazine "Item_SOUL_Unstab";
			player setdamage 1;
			[[player], "fnc_anomaly_dismemUnit", false] call BIS_fnc_MP;
		};