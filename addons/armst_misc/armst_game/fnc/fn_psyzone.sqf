...
while {true} do 
{
	...
	{
		{if ((!alive _x) or (_x isKindOf "ARMST_monolith") or (_psyshield in ["armst_psy_protector_exp","armst_psy_protector"]) )then {_objects=_objects-[_x];};}ForEach _objects;
		{
			if ((_x distance _epicenter)<_range) then
			{
				if (_x isKindOf "CaManBase") then 
				{
					if (local _x) then 
					{
						...
						if (_dmg>1) then {[_x] spawn ARMST_anom_PSY_DEATH;};
					};
				}
				else
				{
	...
	if ((_dist<=_range) and (!(player isKindOf "ARMST_monolith")) ) then 
	{
		...
		if (isNil("_pfx")) then {[] spawn ARMST_anom_PSY_FX};
	};
