...
armst_start_pos = getPosATL player;
...
player setposATL [213.6,-123.729,0.655];
...
BUTTON_1 ctrlSetEventHandler ["ButtonDown","_this spawn fn_dsl_SelfView_RandomizeGear; call fn_dsl_SelfView_UpdateAllText"];
...
fn_dsl_SelfView_Text_Headgear = {TEXT_HEADGEAR ctrlSetText "Головной убор: " + getText (configFile >> "cfgWeapons" >> (dsl_HatTypes select dsl_HeadGearIncrementer) >> "displayName")};
fn_dsl_SelfView_Text_Face = {TEXT_FACE ctrlSetText "Face: " + (dsl_Heads select dsl_Face_Ind);};
fn_dsl_SelfView_Text_Vest = {TEXT_VEST ctrlSetText "Разгрузка: " + getText (configFile >> "cfgWeapons" >>(dsl_VestTypes select dsl_VestIncrementer) >> "displayName")};
...
fn_dsl_SelfView_Text_Backpack = {TEXT_BACKPACK ctrlSetText "Рюкзак: " + getText (configFile >> "cfgVehicles" >> (dsl_BackpackTypes select dsl_BackPackIncrementer) >> "displayName")};
...
fn_dsl_SelfView_Text_Sidearm = {TEXT_SIDEARM ctrlSetText "Оружие: " + getText (configFile >> "cfgWeapons" >> (dsl_WeaponListPistols select dsl_WeaponListPistols_ind) >> "displayName")};
...
fn_dsl_SelfView_Text_Glasses = {TEXT_GLASSES ctrlSetText "Аксессуар: " + (dsl_Glasses select dsl_Glasses_ind)};


fn_dsl_SelfView_RandomizeGear = 
{
closeDialog IDC_CANCEL;
ARMST_SPAWN = [resp_1,resp_2,resp_3,resp_4,resp_5,resp_6,resp_7,resp_8,resp_9,resp_10,resp_11,resp_12,resp_13,resp_14,resp_15,resp_16,resp_17,resp_18,resp_19,resp_20];
...
player linkItem "armst_item_pda";
player setVariable ["Score", 10,true];
...
fn_dsl_SelfView_UpdateBackpack = 
{
	...
	if (dsl_BackPackIncrementer > 0) then
	{
		...
		{	player addBackpack (dsl_BackpackTypes select dsl_BackpackIncrementer);
			...
			player addItemToBackpack "ARMST_spoon";
			player addItemToBackpack "armst_spicki";
			player addItemToBackpack "DAPSTALKERS_BOLTS";
...
fn_dsl_ChangePistol =
{
	...
	{player addMagazine ((getArray (configfile >> "CfgWeapons" >> (dsl_WeaponListPistols select dsl_WeaponListPistols_ind) >> "magazines")) select 0)} forEach [1,2]; 
